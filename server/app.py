...
def load_manager():
    ...
    if session["manager_id"] not in managers:
        managers[session["manager_id"]] = GraphManager()


def get_manager():
    return managers[session["manager_id"]]
...
def change_book():
    ...
    return jsonify({"Book": "Book Changed"})


@app.route("/Addnote", methods=["POST"])
def add_note():
    text = request.json["text"]
    response_data = get_manager().get_graphs_and_scores(
        text=text, start_rank=1, num_graphs=3
    )
...
def reset_all():
    ...
    return jsonify({"reset": "Reset Successful"})
