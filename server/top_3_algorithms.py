...
def SpellCheckerWithUnknown(word, ABBREVIATIONS, MEDICAL_TERMS):
    ...
    if len(misspelled) != 0:
        ...
        if (
            misspelled_word not in ABBREVIATIONS
            and misspelled_word not in MEDICAL_TERMS
        ):
...
def addScore(algorithm_scoring, algorithm_node_scoring, x):
    for y in x:
        ...
        for i in range(0, len(z)):
            if y[1] == z[i][0]:
                algorithm_node_scoring[y[0]][i][1] = (
                    algorithm_node_scoring[y[0]][i][1] + y[2]
                )
                found = True
...
def get_text_score(
    algorithm_scoring, data, input_of_user, algorithm_node_scoring, prev_input
):
